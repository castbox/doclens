...
3. **审阅提效**：支持 **PRD 审阅**与**审阅单**（Review Sheet）创建、结构化记录、状态流转、关联文档。
...
* **Reviewer**：审阅 PRD / 设计文档，创建审阅单、给结论、提问题、跟踪关闭
...
3. 在 PRD 页面 → 一键创建审阅单 → 填写结论/风险/问题项 → 指派责任人 → 跟踪状态直到 Done
4. 在审阅列表页查看历史审阅记录，搜索“某个模块风险”快速复用
...
* 自动带出：
  ...
  * `doc_path`
  * `doc_title`（优先 Markdown H1，否则文件名）
  * `doc_type`（PRD/Design/PR/Log/Other，可改）
...
* 基本信息：
  ...
  * 标题、关联文档、模块/业务线、Owner、Reviewer
* 审阅结论（enum）：

  * `Approved / Changes Requested / Reject / Need Discussion`
* 摘要（必填，短文本）：一句话结论
* 风险清单（可多条）：性能/安全/边界/可维护性/可测试性
* 验收点（可多条）：Reviewer 关心的可验证项
* 问题项（可多条，最关键）：
...
4. 可创建 PRD 审阅单，支持字段填写、问题项、状态流转
5. 具备最小回归保护（单测 + 冒烟步骤）并符合职责拆分结构
